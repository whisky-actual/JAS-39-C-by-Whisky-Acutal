...
HUD_strdefs_text            = {0.008,0.008, 0, 0}
HUD_strdefs_digit           = {0.008,0.008, 0, 0}
HUD_Normal_Size             = {0.0075, 0.0075, 0, 0}
...
local GREEN 		= {0,510,0,510}
--local GREEN 		= {-50,255,-50,1000}
materials["HudGreen"]     = MakeMaterial(nil, GREEN)
...
function create_HUD_locksymbol(HUD_material, UL_X,UL_Y,DR_X,DR_Y, sale,CENTER_X,CENTER_Y)

if sale == nil then
	sale = 1.4
end

local mils_per_pixel =  sale/180
local W 	   		 = DR_X - UL_X
local H 	   		 = DR_Y - UL_Y
local cx		     = (UL_X + 0.5 * W)
local cy		     = (UL_Y + 0.5 * H)
...
local object = CreateElement "ceTexPoly"
	  object.material =  HUD_material
 	  object.vertices =  {{-half_x - dcx, half_y + dcy},
						  ...
						  { half_x - dcx,-half_y + dcy},
						  {-half_x - dcx,-half_y + dcy}}
	  object.tex_coords = hud_texture_box(UL_X,UL_Y,W,H, 360, 360)
...
function AddHudCircle(xpos, ypos, radius, border, fill, parent_element, color)
	...
	for i=1, border do
			...
			if fill == true then
				...
				set_circle	(RWR_circle_i, radius + 1, radius - 1, 360, 360)
			end
...
function add_text_hud(text, posx, posy, OBJparent, font_mat, stringdefs, valign) --used
	if valign == nil then
		valign = "CenterCenter"
	end
	...
	local OBJ_Parent    = CreateElement "ceSimple"
	OBJ_Parent.name     = create_guid_string()
	OBJ_Parent.init_pos = {posx, posy}
	if OBJparent ~= nil then
		OBJ_Parent.parent_element = OBJparent.name
	end
	AddHudElement(OBJ_Parent)

	local Text          = CreateElement "ceStringPoly"
	Text.name           = create_guid_string()
	Text.material       = vfont_mat
	Text.stringdefs     = stringdefs
	Text.alignment      = valign
	Text.value          = text
	Text.parent_element = OBJ_Parent.name
	Text.element_params = {"HUD_BRIGHTNESS"}
